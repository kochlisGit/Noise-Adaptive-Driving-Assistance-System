...
class LKAS:
    ...
    def get_observation(self, data) -> np.ndarray:
        if data is None:
            ...
            self._memory_buffer.appendleft(lane_mask)
            return lane_mask
...
base_model.build(input_shape=[(None, 60, 80, 1), (None, 3)])
...
for i in range(200):
    if np.random.random() < 0.3:
        ...
        data = np.random.randint(0, 256, size=(60 * 80 * 4))
        num_pixels = 100  # Number of pixels to set to [50, 234, 157]
        pixel_indices = np.random.choice(60 * 80, size=num_pixels, replace=False).astype(int)
        for pixel_i in pixel_indices:
            data[pixel_i * 4: (pixel_i + 1) * 4] = [50, 234, 157, 255]
    ...
    lane_mask = lka.get_observation(data)
    s.sample()
