...
def main(
    ...
    for _ in range(num_iterations):
        ...
        agent_controller = AgentController(vehicle=vehicle)
        agent_controller.set_destination(world.get_map().get_spawn_points()[0].location)
        world.tick()
        ...
        while not done:
            ...
            control = agent_controller.get_pid_control()
            vehicle.apply_control(control)

            if control.brake > 0.0:
                input('Found brake > 0.0. Check it')

            if debug:
                print(f'PID Control: {control}')
...
if __name__ == '__main__':
    main(
        ...
        sensor_data_corrupt_prob=0.4,
        segmentation_noise_ratio=0.3,
