...
from nadas.environments.wrappers.stacking import FrameStackingEnv
from nadas.models.actor_critic import ActorCritic
import time
import os
...
def main(
    ...
    ModelCatalog.register_custom_model('ppo_lstm_model', ActorCritic)
    ...
    # agent = agent_config.environment(env=FrameStackingEnv, env_config=env_config).build()
    ...
    # episode_steps = []
    # checkpoints = []
    logs = pd.read_csv('nadas/experiments/logs/ppo_train.csv')
    ...
    while i < num_iterations:
        try:
            ...
            df.to_csv('nadas/experiments/logs/ppo_train.csv', index=False)
            ...
            fig.savefig(f'nadas/experiments/plots/ppo_average_returns.png')
            plt.close()
...
if __name__ == '__main__':
    with tf.device('/gpu:0'):
        ...
        main(
            ...
            num_frames=num_frames,
            num_iterations=500,
            sensor_data_corrupt_prob=0.3,
            segmentation_corrupt_portion_size=(12, 16),
            depth_error_rate=0.05,
            max_steps=1024,
            use_state_prediction=False,
            iterations_per_reload=20,
            action_repeats=15,
            debug=False,
